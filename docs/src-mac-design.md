...
- 排除本机地址、IPv4 子网网络地址/广播地址、无效/未指定/组播/回环来源；重叠前缀且来源接口未知时不探测。

这是根据本机接口地址推定的直连范围，不是真实 ingress 信息或管理员 LAN 白名单，也不能自动分辨 WAN 与 LAN。启用后，符合上述条件的 WAN 直连地址也可能被探测。当前没有额外接口列表配置，不查询默认路由或以网关 MAC 代替来源身份。无编号接口、特殊路由及复杂 VRF 拓扑可能无法补救，应在对应设备上验证。
